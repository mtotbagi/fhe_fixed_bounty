...
use fhe_fixed::{FheFixedU, FixedClientKey, FixedServerKey, FixedSize, FixedFrac, FixedCiphertext};
use std::sync::LazyLock;

use typenum::{U0, U8, U16, U32, U64};
    ...
    F: Fn(&mut FheFixedU<Size, Frac>, &mut FheFixedU<Size, Frac>, &FixedServerKey),
{
    ...
    bench_group.bench_function(&bench_id, |b| {
        ...
        let encrypt_two_values = || {
            ...
            (SKEY.from_bits(ct_0), SKEY.from_bits(ct_1))
        };
    ...
    (method_name: $server_key_method:ident, display_name:$name:ident) => {
        fn $server_key_method(c: &mut Criterion) {
            bench_server_key_unary_function_dirty_inputs::<_, U16, U8>(
                ...
                stringify!($name),
                |lhs, server_key| {
                    server_key.$server_key_method(lhs, 5);
            });
...
define_server_key_bench_unary_fn!(method_name: smart_sqr, display_name: square);
define_server_key_bench_unary_fn!(method_name: smart_dbl, display_name: double);
...
define_server_key_bench_unary_default_fn!(method_name: unchecked_sqr, display_name: square);
define_server_key_bench_unary_default_fn!(method_name: unchecked_dbl, display_name: double);
