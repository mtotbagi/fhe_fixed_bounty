...
impl FixedServerKey {
    ...
    pub fn unchecked_mul<T: FixedCiphertext>(&self, lhs: &T, rhs: &T) -> T {
    ...
    pub fn smart_mul_assign<T: FixedCiphertext>(&self, lhs: &mut T, rhs: &mut T) {
        *lhs = self.smart_mul(lhs, rhs);
    }

    pub fn unchecked_mul_assign<T: FixedCiphertext>(&self, lhs: &mut T, rhs: &T) {
    ...
    pub fn unchecked_sqr<T: FixedCiphertext>(&self, c: &T) -> T {
    ...
    pub fn smart_sqr_assign<T: FixedCiphertext>(&self, c: &mut T) {
        *c = self.smart_sqr(c);
    }

    pub fn unchecked_sqr_assign<T: FixedCiphertext>(&self, c: &mut T) {
...
fn compute_block_sqrs<T: IntegerRadixCiphertext>(c: &T, key: &ServerKey) -> T {
    let message_modulus = key.as_ref().message_modulus.0;
    let lsb_block_sqr_lut = key.as_ref().generate_lookup_table(|x| (x * x) % message_modulus);
    let msb_block_sqr_lut = key.as_ref().generate_lookup_table(|x| {
    ...
    let message_part_terms_generator = rhs
        ...
        .map(|(i, rhs_block)| {
            ...
            result.blocks_mut()[2 * i + 1..]
                ...
                .for_each(|lhs_block| {
                    key.as_ref().unchecked_apply_lookup_table_bivariate_assign(
                        lhs_block,
                        rhs_block,
                        &lsb_block_mul_lut,
                    );
            ...
            result.blocks_mut()[i..2 * i + 1]
                ...
                .filter(|block| block.degree.get() != 0)
                .for_each(|lhs_block| {
                    key.as_ref().create_trivial_assign(lhs_block, 0);
                });
            result
        });
