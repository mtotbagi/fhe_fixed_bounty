...
use tfhe::{
    integer::{BooleanBlock, IntegerCiphertext, IntegerRadixCiphertext},
...
impl FixedServerKey {
    ...
    pub fn unchecked_div<T: FixedCiphertext>(&self, lhs: &T, rhs: &T) -> T {
        let mut result_value: T = lhs.clone();
        self.unchecked_div_assign(&mut result_value, rhs);
        result_value
    }

    pub fn smart_div_assign<T: FixedCiphertext>(&self, lhs: &mut T, rhs: &mut T) {
    ...
    pub fn unchecked_div_assign<T: FixedCiphertext>(&self, lhs: &mut T, rhs: &T) {
        if T::IS_SIGNED {
            ...
            let (_, sign_bits_are_different) = rayon::join(
                ...
                || {
                    let sign_bit_pos = self.key.as_ref().message_modulus.0.ilog2() - 1;
                    ...
                    let lut = self
                        .key
                        .as_ref()
                        .generate_lookup_table_bivariate(compare_sign_bits);
                    BooleanBlock::new_unchecked(
                        self.key.as_ref().unchecked_apply_lookup_table_bivariate(
    ...
    fn unchecked_unsigned_div_assign_rem<T: FixedCiphertext>(
        ...
        if lhs.frac() % 2 == 1 {
            self.key
                .unchecked_scalar_right_shift_assign(&mut wide_remainder, 1);
        }
        ...
        self.key.unchecked_scalar_left_shift_assign_parallelized(
            &mut guess_radix,
            largest_used_bit_idx,
        );
        ...
        let zero_out_if_overflow_lut =
            self.key
                .as_ref()
                .generate_lookup_table(|block| if block & 1 == 0 { block >> 1 } else { 0 });
        ...
        let (shifted_wide_rhs_vec, leading_zeros) = rayon::join(
            || {
                ...
                let wide_rhs = self
                    .key
                    .extend_radix_with_trivial_zero_blocks_msb(&rhs.bits(), blocks_with_frac + 1);
                let wide_shifted_rhs_vec = (0..=message_mod)
                    .into_par_iter()
                    .map(|idx| {
                ...
                (0..=largest_used_bit_idx as isize)
                    .into_par_iter()
                    .map(|idx| {
                        let mod_of_shift = (idx % message_mod + message_mod) % message_mod;
                        let shift_amount_signed = idx - mod_of_shift;
                        let wider_shifted = unchecked_signed_scalar_left_shift_parallelized(
                            &self.key,
                            &wide_shifted_rhs_vec[mod_of_shift as usize],
                            shift_amount_signed,
                        );
                        Bits::from_blocks(
                            wider_shifted.blocks()[..wider_shifted.blocks().len() - 1].to_vec(),
                        )
                    })
                    .collect::<Vec<_>>()
            },
        ...
        for guess_bit_idx in (0..=largest_used_bit_idx).rev() {
            ...
            let mut narrow_remainder_old =
                Bits::from(wide_remainder.blocks()[ls_used_block_idx..].to_vec());
            ...
            let narrow_rhs_shifted = Bits::from(
                shifted_wide_rhs_vec[guess_bit_idx].blocks()[ls_used_block_idx..].to_vec(),
            );
            ...
            let ((mut narrow_remainder_new, sub_overflowed), is_leading_zero) = rayon::join(
                || {
                    self.key.unchecked_unsigned_overflowing_sub_parallelized(
                        &narrow_remainder_old,
                        &narrow_rhs_shifted,
                    )
                },
                || {
                    let total_size = lhs.size() as isize + lhs.frac() as isize
                        - (blocks_with_frac * log_modulus_usize) as isize;
                    let inverse_idx = (largest_used_bit_idx - guess_bit_idx) as isize;
                    let cmp_val = total_size - 1 - inverse_idx;
                    if cmp_val < 1 {
                        self.key.as_ref().create_trivial(0)
                    } else {
                        self.key
                            .unchecked_scalar_lt_parallelized(&leading_zeros, cmp_val as u64)
                            .into_raw_parts()
                    }
                },
            );
            ...
            let overflow_happened = self
                .key
                .as_ref()
                .unchecked_bitor(&sub_overflowed.into_raw_parts(), &is_leading_zero);
            ...
            rayon::join(
                || {
                    ...
                    guess_block = self.key.as_ref().unchecked_add(&guess_block, &guess_block);
                    self.key
                        .as_ref()
                        .unchecked_add_assign(&mut guess_block, &overflow_happened);
                    self.key
                        .as_ref()
                        .apply_lookup_table_assign(&mut guess_block, &zero_out_if_overflow_lut);
                    guess_block.degree = Degree::new(1 << (guess_bit_idx % log_modulus_usize));
                    self.key.as_ref().unchecked_add_assign(
                        &mut wide_result.blocks_mut()[ls_used_block_idx],
                        &guess_block,
                    );
                },
                || {
                    ...
                    rayon::join(
                        || {
                            ...
                            narrow_remainder_old
                                ...
                                .par_iter_mut()
                                .for_each(|block| {
                                    *block = self.key.as_ref().unchecked_add(block, block);
                                    self.key.as_ref().unchecked_add_assign(block, &overflow_happened);
                                    self.key.as_ref().apply_lookup_table_assign(
                                        block,
                                        &zero_out_if_no_overflow_lut,
                                    );
                                });
                        },
                        || {
                            //keep the new if it didn't overflow (R >= B*D) -> zero if overflow
                            narrow_remainder_new
                                .blocks_mut()
                                .par_iter_mut()
                                .for_each(|block| {
                                    *block = self.key.as_ref().unchecked_add(block, block);
                    ...
                    let narrow_remainder_final = self
                        .key
                        .unchecked_add_parallelized(&narrow_remainder_new, &narrow_remainder_old);
            ...
            // compute B >>= 1
            self.key
                .unchecked_scalar_right_shift_assign_parallelized(&mut guess_radix, 1);
        }
        // discard unused part of the result, and return the rest
        let mut narrow_result =
            Bits::from_blocks(wide_result.into_blocks()[..narrow_block_size].to_vec());
        let mut narrow_remainder =
            Bits::from_blocks(wide_remainder.into_blocks()[..narrow_block_size].to_vec());
        rayon::join(
            || {
                narrow_result.blocks_mut().par_iter_mut().for_each(|block| {
                    // while there are no carries, there is some noise that we should clear
                    self.key.as_ref().message_extract_assign(block);
                });
            },
            || {
                narrow_remainder
