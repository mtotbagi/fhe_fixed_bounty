...
impl FixedServerKey {
    ...
    ///
    /// let ct_res = skey.smart_sqrt(&mut a);
    ///
    /// // Decrypt:
    ...
    pub fn unchecked_sqrt_assign<T: FixedCiphertext>(&self, c: &mut T) {
        ...
        let mut wide_remainder = self
            .key
            .extend_radix_with_trivial_zero_blocks_lsb(&c.bits(), blocks_with_frac);
        ...
        let mut guess_square = if used_i_bits > 0 {
            self.key
                .unchecked_scalar_left_shift_parallelized(&guess_radix, used_i_bits - 1)
        } else {
            // used_i_bits is 0
            self.key
                .unchecked_scalar_right_shift_parallelized(&guess_radix, 1)
        };
        ...
        let guess_overflow_lut = self.key.as_ref().generate_lookup_table(|block| {
            (block & (self.key.message_modulus().0 - 2))
                + if (block & 1) == 0 { block >> 2 } else { 0 }
        });
        ...
        for guess_bit_idx in (least_used_bit_idx..=largest_used_bit_idx).rev() {
            ...
            let shift_amount_signed =
                guess_bit_idx as isize - least_used_bit_idx as isize - c.frac() as isize + 1;
            let ls_used_bit_idx = std::cmp::min(
                guess_bit_idx as isize + shift_amount_signed,
                std::cmp::min(sqr_bit_idx, guess_bit_idx) as isize,
            ) as usize;
            ...
            let mut narrow_result_shifted =
                Bits::from(wide_result.blocks()[ls_used_block_idx..].to_vec());
            ...
            narrow_result_shifted = unchecked_signed_scalar_left_shift_parallelized(
                &self.key,
                &narrow_result_shifted,
                shift_amount_signed,
            );
            ...
            self.key.as_ref().unchecked_add_assign(
                &mut narrow_result_shifted.blocks_mut()[sqr_block_idx_narrow],
                &sqr_block,
            );

            // calculate V - (2*R*B + BS) and V >= (2*R*B + BS)      (the new value of V, and the guard of the if-stmt)
            let (mut narrow_remainder_new, overflow_happened) =
            ...
            rayon::join(
                || {
                    ...
                    self.key
                        .as_ref()
                        .unchecked_scalar_mul_assign(&mut guess_block, 4);
                    self.key
                        .as_ref()
                        .unchecked_add_assign(&mut guess_block, &overflow_happened);
                    self.key.as_ref().unchecked_add_assign(
                        &mut guess_block,
                        &wide_result.blocks()[guess_block_idx],
                    );
                    self.key
                        .as_ref()
                        .apply_lookup_table_assign(&mut guess_block, &guess_overflow_lut);
            ...
            self.key
                .unchecked_scalar_right_shift_assign_parallelized(&mut guess_square, 2);
            sqr_bit_idx -= 2;
        }
