...
impl FixedServerKey {
    ...
    pub fn smart_round<T: FixedCiphertext>(&self, c: &mut T) -> T {
        ...
        if T::IS_SIGNED {
            ...
            let sign_bit_lut = self.key.as_ref().generate_lookup_table(|x| {
                let x = x % 4;
                (x >> 1) & 1
            });
            ...
            let sign_bit = self.key.as_ref().apply_lookup_table(last_block, &sign_bit_lut);
    ...
    pub fn smart_trunc<T: FixedCiphertext>(&self, c: &mut T, prec: usize) -> T {
        ...
        if bits_to_lose % 2 == 1 {
            ...
            let acc = self.key.as_ref().generate_lookup_table(|x| x & 0b10);

            let ct_res = self.key.as_ref().apply_lookup_table(&block[0], &acc);
    ...
    fn round_tie_to_plus_infinity<T: FixedCiphertext>(&self, c: &mut T) -> T {
        ...
        let to_add_lut = self.key.as_ref().generate_lookup_table(|half_block| {
            if T::FRAC % 2 == 1 {
                (half_block & 1) << 1
            } else {
                (half_block >> 1) & 1
        ...
        let (to_add_bits, mut truncated_c) = rayon::join(
            || {
                let to_add_block = self.key.as_ref().apply_lookup_table(half_block, &to_add_lut);
